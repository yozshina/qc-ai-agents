...
**Trạng thái:** Close — Monitor đã trả lời
...
	Đúng theo đề xuất, ko đóng khi click ra ngoài, phải click button
...
	Height padding right 16px, left 16px / font: Roboto (dung chung tất cả hệ thống -> ko cần hỏi lại) / separator: coi mỗi menu là 1 button -> có vùng hiển thị tương ứng và đc fill theo độ dài text, còn chiều cao thì fixed / active color: background Conditional / header-background (#FFFFFF) và chữ: Character/Title .85 (#000000 85%) còn ảnh thì không cần check
...
	Height padding top 1px right 8px bottom 1px left 8px / border radius 16px / color: background polar green/1 (#F6FFED) and chữ Polar Green / 3 (#B7EB8F)
...
	Height 1440*66.18 / content: ảnh / background Conditional/page-background (#F0F2F5)/ typography: Roboto 400 Regular size 12px line height 16px letter spacing 0% / color: Character/Primary .85 cho chữ Autolake và Character/Secondary .45 cho - Copyright ©️ 2026 - Devoloped by Viettel Group
...
	Không dùng với mobile, chỉ dùng trên pc/laptop
...
	Không darkmode gì hết, chỉ sử dụng 1 kiểu duy nhất như figma làm
...
	Chỗ này chỉ rõ ở phần nào, tôi chưa catchup được
...
	Font: Roboto, size 14px, weight: 500, style: Medium
...
	Xác nhận bỏ qua vì phần này đã làm và test done, chỉ cần 1 testcase kiểu dạng basline cũ
...
	Chữ trên input có 2 thành phần thì chữ là Character/Title .85 (#000000 85%) và ký tự require (*) là Dust Red/6 (#F5222D) / error ko đổi thành đỏ, khi error sẽ giống như khi click vào input field là border sẽ đỏ, ngoài ra error message hiển thị dạng helptext ngay bên dưới và màu sắc cũng được gán Dust Red/6 (#F5222D)
...
	Check lại kỹ đi, các action như default, hover, active, focused, disabled có đủ đó. Và cách dạng button khác nhau
...
	Có thay đổi icon set chứ ko mặc định là kính lúp / icon lấy từ system ant design / icon size là 20px*20px
...
	Có pagination, theo common rule hết (common rule của pagination đã được để cập ở file figma Local system -> Phân trang)
