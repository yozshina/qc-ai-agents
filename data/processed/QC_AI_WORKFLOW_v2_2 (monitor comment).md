...
| **Risk-based** | Thay coverage phẳng bằng Risk Score → dồn lực vào vùng nguy hiểm. | Nhưng không đồng nghĩa với vùng ko nguy hiểm được bỏ qua hoặc làm cho xong
| **Monitor-first** | Mọi quyết định về OBSOLETED/REGRESSION/ghi đè đều qua Monitor. AI chỉ đề xuất, không chốt. | Người monitor là người kiểm soát toàn bộ quy trình, mọi quyết định, đề xuất phải được người monitor duyệt. Tuy nhiên người monitor sẽ có quyền chỉ định 1 vài phần đc bypass khi agents đã trải qua đủ nhiều dữ liệu, đã thực hiện tốt, nhưng vẫn sẽ phải tổng hợp đầy đủ dữ liệu ra file, bất chợt người monitor kiểm tra thì phải có và điều chỉnh nếu ko hợp lý/sai/làm kém
| **Resilient** | Thiếu input/MCP lỗi → Degraded Mode, không đứng pipeline, không re-run toàn bộ. | Dừng ngay tại bước không thể tiếp tục, thông báo tới người monitor, khi nào có thể chạy tiếp thì mới next step (bao gồm cả step B0 -> B5 và các step nhỏ/cực nhỏ trong các bước)
...
│  - project-config.json: domain, schema, glossary (và
 có thể yêu cầu AI điều chỉnh mà ko cần vào file config)   │
...
| **Monitor (Senior Tester)** | Gác gate, confirm/reject output AI, trả lời Q&A, duyệt diff thôi (màu đã chỉ định rồi), chỉ định template | Gõ tay TC, làm phần AI có thể tự động hóa |
...
| ✅ | **Dừng tại bước, không re-run toàn bộ** | Tiết kiệm token, giữ trạng thái | Phần này cần bổ sung: Dừng tại bước không thể thực hiện tiếp, AI buộc phải dừng và cảnh bảo người monitor về vấn đề đang phát sinh, tuyệt đối không được chạy tiếp, đặc biệt là việc tính toán/phân tích để sinh ra thêm từ dẫn tên tốn token (tuân thủ 100% pipeline)
...
| ⚠️ | **Thiếu phần execution/defect** (tạm bỏ qua) | Kết thúc ở handoff; không có vòng đời bug | *Scope hiện tại: tester tự thực hiện qua VDI. Sẽ mở rộng sau.* | Chỗ này pending chưa làm, bỏ ra khỏi điểm yếu. Nhưng đẩy vào phần pending chưa làm tới, nghĩ là them phần 1.4 Pending – Chưa thực hiện, bao gồm: execute test, kết nối jira, tự động tạo bug, tự động quản lý bug, tự động collect thông tin trên jira… và 1 số phần nữa nhờ agents note thêm

### 1.3 Rủi ro hệ thống

| Rủi ro | Mức độ | Cách xử lý trong v2 |
|---|---|---|
| Garbage in, garbage out | 🔴 Cao | B0 bắt buộc validate input; [ASSUMPTION] bắt buộc thành Q&A | Validate input ở đây là quyền truy cập, khi truy cập được thì agents có thể tự do lấy thông tin trong scope đc chỉ định và tất nhiên chưa rõ hay thấy thiếu thì biến thành Q&A cho input, validate input là việc bắt buộc phải truy cập đc vào các input người monitor cung cấp để phục vụ cho việc extract dữ liệu

| Over-trust AI → mất coverage | 🔴 Cao | Màu diff bắt buộc; Monitor xác nhận trước khi ghi đè baseline | 

| Q&A không được trả lời → pipeline đứng | 🟡 Trung bình | Cho phép "pending" nhưng không bỏ sót; tiếp tục bước khác nếu Q&A không critical | Chỗ này cần phải đánh giá them là Q&A chưa trả lời đc thì có ảnh hưởng đến A/B/C ko? Nếu ảnh hưởng đến nhỏ thì có thể tạm thời pending nhưng người monitor có thể trả lời sau, lúc đó tính là CR chẳng hạn, còn nếu ảnh hưởng lớn thì phải buộc trả lời đủ Q&A hoặc người monitor confirm bỏ qua thì agents mới được action tiếp
...
| B1 Extract | Bỏ hoặc chỉ đọc vùng nhỏ (chỗ này vẫn nên đọc đầy đủ, vì ko đọc đầy đủ sao biết là lightweight???)| ✅ Đầy đủ | ✅ Đầy đủ |
...
Chỗ phần này tôi cần agents đánh giá them việc B3 TCD và B4 Checklist có trùng nhau ko? Theo tôi thấy B4 checklist đang trung 95% với B3 TCD rồi, yêu cầu agents phân tích cụ thể và đưa ra đánh giá trước cho người monitor chứ ko đc lược bỏ ngay lập tức (đánh giá phải đầy đủ khía cạnh)
...
| **Diff màu** | 🟡 VÀNG = nội dung SỬA; 🟢 XANH LÁ = THÊM MỚI; Đổi thành đỏ hồng  = đề xuất OBSOLETED. Tất cả = CHỜ DUYỆT. |
...
**Nguyên tắc CR:**
- Không re-run toàn pipeline từ B0 (không phải là re-run từ B0 mà ko phân tích lại toàn bộ scope như kiểu 1 job mới, mà là dựa trên job cũ đã có, ai tổng hợp, tóm tắt lại toàn bộ trước đi khi kết thúc bước 5, sau đó đọc lại file đã tóm tắt + CR mới để xử lý). Nếu chỉ là CR thì phải dựa vào cái cũ, còn nếu CR quá lớn -> Chuyển thành job mới, cái này đo người monitor quyết định
...
| **Thiếu file mô tả yêu cầu** | Báo thiếu, yêu cầu Monitor cung cấp. Không suy luận. | Dừng B0, chờ input | Không đúng yêu cầu rồi, người monitor sẽ input toàn bộ những gì có vào luôn, việc của agents tại đây là truy cập vào, phải truy cập thành công -> mới được xử lý đọc dữ liệu dể extract. Giờ ko truy cập đc, dựa vào mỗi input, ví dụ như cái ảnh mờ tịt, thì sao biết đc chi tiết là gì?
...
AI sinh output mới
  ...
  → Tô màu diff:
      ...
      🟢 XANH LÁ (E2EFDA)  = THÊM MỚI
      Đổi thành đỏ hồng cho dễ nhìn  = ĐỀ XUẤT OBSOLETED
...
| **Gate 1** — Extract | Extract đủ 7 hạng mục + Monitor confirm | AI extract lại đúng vùng | Chỗ này viết ra khung chat giúp tôi 7 hạng mục extract là gì với nhé!!! ĐỪNG BỎ SÓT
...
Cấu trúc này tôi thấy vớ vẩn nhé

1. Phần thư mục jobs phải đẩy là thư mục con của thực mục {project-id} chứ?
2. Cấu trúc file phải có version, file kaizen cũng vậy, cấu trúc phải phù hợp, và file knowledge-pipeline cũng vậy (Đặt tên ddown giản lại, thêm hậu tố nhận biết là file được sinh ra sau job nào thuộc project nào, sắp xếp theo thứ tự nữa, cũng có version, ví dụ: improve_v2 + lakehouse-common-rule-ui-sprint14…)
