# QC AI-Agent Workflow — Generic Edition (CORE)
> **Phiên bản tổng quát — Không phụ thuộc bất kỳ dự án nào**
> **Trạng thái: CORE CHÍNH THỨC** (promoted từ v3 — Monitor duyệt áp dụng ngày 2026-06-25)
> File version: `qc-workflow_core_v3_2026-06-25.md`
> Nguồn gốc: v1 (Lakehouse) → v2 (Generic) → v3 (+ comment Monitor) → CORE
>
> ⚠️ **Ghi chú promotion:** Monitor ưu tiên "hoàn thành hơn hoàn hảo" do chưa trải qua đủ job/dự án.
> Các đề xuất trong `review_QC_AI_WORKFLOW_v3` (R-01 → R-23) **được ghi nhận nhưng CHƯA áp dụng** —
> sẽ cân nhắc đưa vào sau khi tích lũy đủ dữ liệu thực tế qua các job. Xem ROADMAP (Section 15) và file review để tra cứu.
>
> 📌 **Quy ước version mới (Monitor chốt 2026-06-25):** Kết hợp `_v{N}_{YYYY-MM-DD}`.
> Ví dụ: `step3-tcd_v1_2026-06-25_C360_dashboard-su-kien-truyen-hinh.md`.
> Quy ước này thay thế cả "by date" (config LKH cũ) lẫn "_v2 suffix" (v3 cũ).

---

> **HẾT PHẦN HEADER PROMOTION — Toàn bộ nội dung bên dưới giữ NGUYÊN từ v3 đã được Monitor duyệt.**
> **Chỉ những chỗ liên quan trực tiếp tới quy ước đặt tên file mới được điều chỉnh cho nhất quán (đánh dấu 🔧).**
...
| **Risk-based** | Thay coverage phẳng bằng Risk Score → dồn lực vào vùng nguy hiểm. **Risk LOW không đồng nghĩa với bỏ qua hoặc làm cho xong. LOW vẫn phải có TC smoke test đủ để tự tin không có lỗi cơ bản.** | LOW ≠ skip |
| **Monitor-first** | Mọi quyết định, đề xuất đều phải được Monitor duyệt. Monitor là người kiểm soát tuyệt đối toàn bộ quy trình. **Tuy nhiên, Monitor có quyền chỉ định bypass một số bước cụ thể khi AI đã thực hiện tốt qua nhiều job — xem Trust Level bên dưới.** | Trust Level / Bypass |
| **Resilient** | Thiếu input / MCP lỗi → Degraded Mode. **Dừng ngay tại điểm không thể tiếp tục — bao gồm cả micro-step nhỏ nhất trong B0→B5. Thông báo tới Monitor. Chờ Monitor xử lý xong mới next step.** | granular stop |

---

### Cơ chế Trust Level — Bypass có điều kiện
...
> **Trạng thái áp dụng cho C360 (job đầu tiên):** Trust Level = 0. Mọi gate đều phải pass thủ công.
...
## 01 — PHÂN TÍCH & ĐÁNH GIÁ WORKFLOW (từ v1 → CORE)
...
| ✅ | **Dừng tại bước lỗi, không re-run toàn bộ** | AI buộc phải dừng cứng — kể cả micro-step — khi gặp vấn đề. Cảnh báo Monitor. Tuyệt đối không tự tính toán/phân tích thêm khi đã dừng (tốn token vô nghĩa). Pipeline chỉ tiếp tục sau khi Monitor xác nhận vấn đề đã được giải quyết. |
...
| ⚠️ | **Gắn chặt vào dự án cụ thể** | Tách core/ + projects/{project-id}/ |
...
#### Phân loại Q&A theo impact level
...
- ✅ Tiết kiệm 1 bước + 1 gate; giảm overhead; không review trùng. Pipeline ngắn hơn, token tiết kiệm hơn.
- ⚠️ TCD format technical → Monitor đọc phức tạp hơn tại Gate 3. Mất checkpoint "dừng trước khi sinh TC".
- 📌 Phù hợp: Monitor có kinh nghiệm với format TCD; dự án mature.

**Phương án 2 — Giữ Checklist nhưng differentiate rõ ràng**
Checklist = bridge từ TCD → TC = TCD + "test approach sơ bộ".
Format mới: `STT | TCD_Ref | Mục_tiêu | Approach (cách test ngắn gọn) | Data_cần | Expected`
- ✅ Tạo giá trị riêng; Checklist trở thành TC skeleton; Monitor review sớm phát hiện sai hướng.
- ⚠️ AI làm thêm 1 bước thực chất; nặng hơn PA1.
- 📌 Phù hợp: dự án mới; domain phức tạp; Monitor muốn kiểm soát chặt.

**Phương án 3 — Tái định nghĩa thành "TC Skeleton"**
B4-before sinh TC Skeleton = bộ TC draft chỉ có header.
Format: `TC_ID | TCD_Ref | Tên_TC | Action_summary | Expected_summary | Risk_inherited`
- ✅ Giá trị gate 4a rõ ràng nhất; Monitor add/remove TC trước khi AI fill detail; traceability TCD→TC tốt.
- ⚠️ AI sinh thêm 1 artifact intermediate; phức tạp hơn PA1.
...
> ⚠️ **Ghi chú CORE (chưa áp dụng — chờ Monitor):** review R-12 chỉ ra 7 hạng mục này thuần UI, có thể không hợp với dự án data/API. C360 Dashboard là dự án có cả UI + data viz → hạng mục này **cơ bản vẫn dùng được**, nhưng job C360 nên bổ sung extract về **data binding / data source của widget dashboard** (xem ghi chú trong job C360).
...
> **Áp dụng theo luồng đã được Monitor confirm tại Gate 0.** Xem phân tích đầy đủ tại Section 02B.
...
> ⚠️ **Ghi chú CORE:** Phần định nghĩa command đầy đủ nằm ngay tại đây trong file core.
> Folder `core/commands/` (mỗi command 1 file chi tiết) **CHƯA được tạo** vì chưa có nội dung chi tiết vượt ngoài bảng dưới.
> Khi nào cần mô tả sâu hơn từng command (ví dụ pseudo-logic, ví dụ input/output thực tế) thì mới tạo folder đó.
...
## 10 — CẤU TRÚC THƯ MỤC (Hybrid — đã đối chiếu repo thực tế)

> 🔧 **Cập nhật CORE (2026-06-25):** Cấu trúc dưới đây đã được điều chỉnh theo quyết định **Hybrid** của Monitor —
> giữ layout repo hiện tại (`common/`, `jobs/`) đang chạy, đồng thời thêm `core/` và `projects/` mới, migrate dần.
> File cũ KHÔNG bị xóa. Xem CHANGELOG mapping ở `projects/` để tra tên cũ → tên mới.
>
> 🟢 **Cập nhật đợt 2 (2026-06-25):** Mục 5 CHANGELOG đã thực hiện — job LKH đã MOVE vào `projects/LKH/jobs/`,
> config LKH đã copy vào `projects/LKH/`. Sơ đồ dưới phản ánh trạng thái dự kiến; trạng thái thực tế xem CHANGELOG.

```
QC-AI-Agents/
│
├── core/                                    ← MỚI · KHÔNG thay đổi theo dự án
│   ├── pipeline/
│   │   ├── qc-workflow_core_v3_2026-06-25.md   ← File này (CORE chính thức)
│   │   └── _archive/qc-workflow_v1_lakehouse_origin.md  ← copy v1 gốc
│   └── rules/
│       └── (chưa tạo — sẽ tách khi có nội dung rule chi tiết)
│   # core/commands/ CHƯA tạo — chưa có nội dung thực (xem Section 09)
│
├── projects/                                ← MỚI · mỗi dự án 1 thư mục
│   ├── _CHANGELOG_file-rename-mapping.md     ← Lịch sử đổi tên file cũ → mới
│   │
│   ├── LKH/                                 ← Dự án Lakehouse
│   │   ├── project-config.json              ← copy từ common/config (bản gốc giữ nguyên)
│   │   ├── knowledge/
│   │   │   ├── pipeline-summary_v1_2026-06-19_LKH_update-ui-common-rule-sprint-14.md
│   │   │   ├── kaizen_v1_2026-06-19_LKH_update-ui-common-rule-sprint-14.md
│   │   │   └── review_v0_LKH_workflow-lakehouse.md
│   │   └── jobs/
│   │       └── LKH_Update-UI-common-rule-sprint-14/   ← MOVED từ jobs/ (16 file đổi tên)
│   │
│   └── C360/                               ← Dự án MỚI: C360 Dashboard
│       ├── project-config.json             ← Config riêng C360 (template, chờ Monitor điền thật)
│       ├── knowledge/                       ← (rỗng — sinh sau khi job đầu DONE)
│       └── jobs/
│           └── C360_Dashboard-Su-Kien-Truyen-Hinh/
│               ├── 00_meta_v1_2026-06-25_C360_dashboard-su-kien-truyen-hinh.md
│               └── input/                   ← (rỗng — chờ Monitor cấp input)
│
├── common/                                  ← GIỮ NGUYÊN (đang chạy thực tế)
│   ├── config/project-config.json           ← Config LKH thật (KHÔNG động vào)
│   ├── processes/knowledge-pipeline/
│   │   └── QC_AI_Workflow_Lakehouse_v1.md   ← File v1 gốc (KHÔNG động vào)
│   ├── templates/ · scripts/ · best-practice/
│
├── jobs/                                     ← Folder rỗng giữ lại (job đã move sang projects/LKH/)
│
└── data/processed/                           ← GIỮ NGUYÊN (chứa v2_2, v3, review v3)
```

### Naming Convention (🔧 Monitor chốt 2026-06-25: kết hợp version số + ngày)

| Loại file | Pattern | Ví dụ |
|---|---|---|
| Workflow core | `qc-workflow_core_v{N}_{YYYY-MM-DD}.md` | `qc-workflow_core_v3_2026-06-25.md` |
| Meta | `00_meta_v{N}_{YYYY-MM-DD}_{project-id}_{job-id}.md` | `00_meta_v1_2026-06-25_C360_dashboard-su-kien-truyen-hinh.md` |
| Artifact bước | `{step}_v{N}_{YYYY-MM-DD}_{project-id}_{job-id}.{ext}` | `step3-tcd_v1_2026-06-25_C360_dashboard-su-kien-truyen-hinh.md` |
| Output TC | `tc-export_v{N}_{YYYY-MM-DD}_{project-id}_{job-id}.xlsx` | `tc-export_v1_2026-06-25_C360_dashboard-su-kien-truyen-hinh.xlsx` |
| Kaizen | `kaizen_v{N}_{YYYY-MM-DD}_{project-id}_{job-id}.md` | `kaizen_v1_2026-06-19_LKH_update-ui-common-rule-sprint-14.md` |
| Pipeline summary | `pipeline-summary_v{N}_{YYYY-MM-DD}_{project-id}_{job-id}.md` | `pipeline-summary_v1_2026-06-19_LKH_update-ui-common-rule-sprint-14.md` |
| CR | `cr_v{N}_{YYYY-MM-DD}_{job-id}.md` | `cr_v1_2026-06-26_dashboard-su-kien-truyen-hinh.md` |

**Quy tắc version:** `v1` = lần đầu sinh. `v2`, `v3`... = sau mỗi lần CR hoặc loop gate. Ngày = ngày sinh/cập nhật file đó. Không xóa version cũ.

---

### project-config.json (Template generic — bản tham chiếu)

> Bản config thực của LKH đang ở `common/config/project-config.json` (giữ nguyên).
> Bản dưới là TEMPLATE tham chiếu cho dự án mới (C360 dùng bản này, Monitor điền giá trị thật).
...
```json
{
  ...
  "domain": {
    "type": "{web_app | mobile_app | data_pipeline | api | dashboard | hybrid}",
    "description": "{Mô tả ngắn domain nghiệp vụ}",
    "glossary": { "{term_1}": "{định nghĩa}" },
    "impact_map": {
      "tầng_B_indirect": "{Định nghĩa vùng indirect — ví dụ: widget dùng chung / data source chung}",
      "tầng_C_workflow": "{Định nghĩa chuỗi workflow — ví dụ: filter → query → render}"
    }
  },

  "input": {
    "required": ["{Loại input bắt buộc}"],
    "optional": ["{Loại input optional}"],
    "design_tool": "{figma | zeplin | sketch | pdf | none}"
  },

  "risk_criteria": {
    "high_risk_areas": ["{Vùng rủi ro cao}"],
    "historical_bugs": ["{Vùng đã từng có bug/escape}"],
    "weight_overrides": { "business_impact": 0.40, "usage_frequency": 0.35, "bug_history": 0.25 }
  },

  "templates": {
    "tc_template": "{path}", "qa_template": "{path}", "export_template": "{path}"
  },
  ...
  "pipeline_rules": {
    ...
    "max_qa_per_round": 10,
    "gate_bypass": { "gate_1_extract": false, "gate_4a_checkpoint": false }
  }
}
```
...
**1. Review pipeline thực tế** — Bước nào tốn thời gian bất thường? Gate nào loop nhiều? AI sai/thiếu ở đâu? Phân loại luồng đúng không? Trust Level cần điều chỉnh?
**2. Review Q&A** — Pattern lặp lại → bổ sung glossary/high_risk_areas. Q&A thừa → cập nhật input checklist. Q&A CRITICAL từ thiếu domain → cập nhật impact_map.
**3. Review Impact** — Vùng nào bỏ sót impact B/C → bổ sung impact_map. Rule core nào cần chỉnh?
**4. Review Risk Score** — Phân bổ effort có đúng thực tế? Trọng số cần chỉnh? Bug ở vùng LOW → cập nhật historical_bugs.
**5. Review B4-before** — PA đã dùng có tạo giá trị? Cần switch PA?
**6. Cập nhật file** — Ghi `00_meta`; tạo `kaizen_v{N}_{date}_{project}_{job}.md`; tạo `pipeline-summary_v{N+1}_...`; cập nhật `project-config.json`.
...
### Setup (1 lần)
```
1. Copy core/ vào môi trường (không sửa)
2. Tạo projects/{project-id}/project-config.json → điền domain, glossary, impact_map, risk, template paths; trust_level: 0
3. Chuẩn bị template TC, Q&A, Export do Monitor chỉ định
4. Tạo projects/{project-id}/jobs/{job-id}/00_meta_v1_{date}_{project-id}_{job-id}.md
```
...
- Config có thể đã cập nhật từ kaizen → domain hiểu sâu hơn, Q&A ít hơn
- Baseline job trước reuse trong reuse_map
...
□ Đặt tên file theo quy ước: _v{N}_{YYYY-MM-DD}_{project-id}_{job-id}
```
...
| **v3** | Trust Level/Bypass; phân loại Q&A; B4-before 3 PA; cấu trúc thư mục; màu ĐỎ HỒNG; naming convention; CR đầy đủ | ✅ Done |
| **CORE (hiện tại)** | Promote v3 thành core chính thức; áp quy ước version kết hợp; hybrid folder; áp dụng cho C360 | ✅ Áp dụng |
| **Chờ xử lý (từ review v3)** | 23 đề xuất R-01→R-23 — đặc biệt R-12 (extract profile theo domain), R-14/R-21 (atomic step cho Manual-MCP), R-06 (test data) | 🔲 Sau khi đủ dữ liệu job |
| **v4** | Áp 3 PA B4-before thực tế; Trust Level auto; common rule baseline inject | 🔲 Kế hoạch |
| **v5+** | Execution-Ready TC format; Manual-MCP; Automation framework; Jira | 🔲 Tương lai (xem 1.4 PENDING) |

---

*QC_AI_WORKFLOW — CORE Edition (promoted từ v3)*
*Không chứa thông tin cụ thể của bất kỳ dự án nào.*
*Config dự án: `projects/{project-id}/project-config.json` (hoặc `common/config/` cho LKH).*
*Lõi pipeline: `core/pipeline/qc-workflow_core_v3_2026-06-25.md`*
*Đề xuất chưa áp dụng: tra cứu `data/processed/review_QC_AI_WORKFLOW_v3.md`*
